...
## GML Editor Panel - Main screen editor for HTML/CSS files attached to GtmlView nodes.
...
const GtmlSearchEngineScript = preload("res://addons/gtml/src/editor/GtmlSearchEngine.gd")
const GtmlJumpResolverScript = preload("res://addons/gtml/src/editor/GtmlJumpResolver.gd")
const GtmlEditorContextScript = preload("res://addons/gtml/src/editor/GtmlEditorContext.gd")
const GtmlColorTokensScript = preload("res://addons/gtml/src/editor/GtmlColorTokens.gd")
const GtmlAutocompleteSourceScript = preload("res://addons/gtml/src/editor/GtmlAutocompleteSource.gd")
...
# Editor state preservation (per GtmlView node path)
...
func _setup_panel() -> void:
	...
	# and route every request through GtmlAutocompleteSource.
	for ce in [html_code_edit, css_code_edit]:
...
func _add_caret_at_next_match(code_edit: CodeEdit) -> void:
	...
	var matches: Array = GtmlSearchEngineScript.find_all(code_edit.text, selected, true, false)
	if matches.size() < 2:
		return
...
## Edit a GtmlView node - loads its HTML/CSS files into the editor
func edit_gtml_view(gtml_view: Node) -> void:
	if gtml_view == null:
		_show_no_selection()
		return
	...
	if _has_unsaved_changes():
		var current_view = _current_gtml_view.get_ref()
		if current_view != null and current_view != gtml_view:
			_show_unsaved_dialog(func():
				_do_edit_gtml_view(gtml_view)
			)
			return
...
## Clear the editor (called when GtmlView is deselected)
...
func _do_edit_gtml_view(gtml_view: Node) -> void:
	# Save state for previous view before switching
	_save_editor_state()

	_current_gtml_view = weakref(gtml_view)
	_html_path = gtml_view.html_path
	_css_path = gtml_view.css_path
...
func _save_editor_state() -> void:
	var gtml_view = _current_gtml_view.get_ref()
	if not gtml_view or not is_instance_valid(gtml_view):
		return

	var node_path = str(gtml_view.get_path())
...
func _update_ui() -> void:
	...
	# Update file label
	if gtml_view:
		file_label.text = gtml_view.name
	else:
		file_label.text = "(no GtmlView selected)"
...
func _save_files() -> void:
	...
	if _html_dirty and not _html_path.is_empty():
		...
		if file:
			...
			_html_dirty = false
			saved_any = true
			print("GtmlEditorPanel: Saved HTML to ", absolute_path)
		else:
			var error = FileAccess.get_open_error()
			push_error("GtmlEditorPanel: Failed to save HTML to %s (error: %d)" % [absolute_path, error])
	...
	if _css_dirty and not _css_path.is_empty():
		...
		if file:
			...
			_css_dirty = false
			saved_any = true
			print("GtmlEditorPanel: Saved CSS to ", absolute_path)
		else:
			var error = FileAccess.get_open_error()
			push_error("GtmlEditorPanel: Failed to save CSS to %s (error: %d)" % [absolute_path, error])

	if saved_any:
		_update_tab_titles()
		_update_save_button()
		# GtmlView will auto-detect file changes via modification time
		_refresh_warnings()
...
func _refresh_warnings() -> void:
	...
	if not html_code_edit.text.is_empty():
		var html_parser = GtmlHtmlParserScript.new()
...
func _on_replace_pressed() -> void:
	...
	code_edit.text = GtmlSearchEngineScript.replace_one(code_edit.text, engine_match, replace_input.text)
	_perform_search()
	_on_search_next()
...
func _on_replace_all_pressed() -> void:
	var code_edit := _get_active_code_edit()
	var result: Dictionary = GtmlSearchEngineScript.replace_all(
		code_edit.text,
		search_input.text,
		replace_input.text,
...
func _perform_search() -> void:
	...
	var code_edit = _get_active_code_edit()
	var engine_matches: Array = GtmlSearchEngineScript.find_all(
		code_edit.text,
		search_text,
		match_case_check.button_pressed,
		regex_check.button_pressed,
...
func _trigger_jump_at_caret(code_edit: CodeEdit) -> void:
	...
	var ctx: GtmlEditorContext
	if kind == "html":
		ctx = GtmlEditorContextScript.from_html(
			code_edit.text,
		...
		ctx = GtmlEditorContextScript.from_css(
			code_edit.text,
			code_edit.get_caret_line(),
			code_edit.get_caret_column(),
			other,
	...
	var jump = GtmlJumpResolverScript.resolve(ctx)
	if jump == null:
		return
...
func _rescan_colors(kind: String) -> void:
	...
	if code_edit == null:
		return
	var tokens: Array = GtmlColorTokensScript.scan(code_edit.text)
...
func _write_back_color(code_edit: CodeEdit, token: Dictionary, new_color: Color) -> void:
	var new_literal := GtmlColorTokensScript.format(new_color, token["kind"])
...
#region Autocomplete (CodeEdit native popup wired to GtmlAutocompleteSource)
...
func _on_code_completion_requested(code_edit: CodeEdit) -> void:
	...
	var candidates: Array = GtmlAutocompleteSourceScript.get_candidates(ctx)
