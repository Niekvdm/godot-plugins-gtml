class_name GtmlFocusManager
extends RefCounted

## Keyboard/gamepad focus traversal for a built GTML Control tree.
##
## Reads the _gtml_* focus meta stamped at build time by the renderer
...
## and does NOT grab focus (GtmlView orchestrates autofocus once per
...
const META_FOCUSABLE := "_gtml_focusable"
const META_TABINDEX := "_gtml_tabindex"
const META_TAB_SKIP := "_gtml_tab_skip"
const META_AUTOFOCUS := "_gtml_autofocus"
const META_TRAP := "_gtml_focus_trap"
...
## First focusable with _gtml_autofocus in document order, or null.
