...
## Percent-based sizing and max-size enforcement live in GmlPercentSizing.


## Apply width / height / min-* / max-* / flex-* / cursor onto a control.
## Returns nothing; metadata used by GmlPercentSizing is stamped on the control.
static func apply(control: Control, style: Dictionary) -> void:
	...
	if (width_percent > 0 and width_percent < 1.0) or (height_percent > 0 and height_percent < 1.0):
		...
		GmlPercentSizing.attach(control)
	...
	if has_max:
		GmlPercentSizing.attach_max_size(control)
