@tool
class_name GtmlRenderer
extends RefCounted

const GtmlBindingParserScript = preload("res://addons/gtml/src/binding/GtmlBindingParser.gd")
const GtmlBindingApplierScript = preload("res://addons/gtml/src/binding/GtmlBindingApplier.gd")
const GtmlBindingExprScript = preload("res://addons/gtml/src/binding/GtmlBindingExpr.gd")
...
## ``GtmlDimensions`` / ``GtmlWrap`` / ``GtmlBackgrounds`` / ``GtmlStyles``,
## resize-driven sizing lives in ``GtmlPercentSizing``, and pseudo-class
## interpolation lives in ``GtmlTransitionSetup`` + ``GtmlTransitionManager``.


var _gtml_view = null  # GtmlView reference
var _styles: Dictionary = {}
var _defaults: Dictionary = {}
var _transition_manager: GtmlTransitionManager = null
## Ancestor chain (Array of GtmlNode, root-first) of the node currently
...
func build(root, styles: Dictionary, gtml_view) -> Control:
	_gtml_view = gtml_view
	_styles = styles
	_defaults = gtml_view.get_tag_defaults()
	_transition_manager = GtmlTransitionManager.new()
...
func _build_node(node, ancestor_chain: Array = []) -> Control:
	...
	if node.attrs.has("v-if") and _gtml_view != null and _gtml_view.state != null:
		var v_if_src: String = node.attrs["v-if"]
		var v_if_scope: Dictionary = node.get_meta("_binding_scope", {})
		if not GtmlBindingApplierScript.eval_v_if(v_if_src, _gtml_view.state, v_if_scope):
			return null
	...
	GtmlDimensions.apply(control, _get_node_style(node))
	...
	if not (inner is Button):
		...
		GtmlTransitionSetup.setup(transition_target, _get_node_style(node), _transition_manager)
...
func _register_bindings_for_node(node, control: Control, inner: Control = null) -> void:
	if _gtml_view == null or control == null:
		return
	var registry = _gtml_view._binding_registry
	if registry == null:
		return
	var state = _gtml_view.state
	if state == null:
		return
	...
	for attr_name in node.attrs:
		var cls: Dictionary = GtmlBindingParserScript.classify_attribute(attr_name)
		match cls["kind"]:
			"v-bind":
				var expr: Dictionary = GtmlBindingExprScript.parse(node.attrs[attr_name])
				if cls["target"] == "class":
					var restyle_cb := _make_class_restyle_callback(control, node)
					GtmlBindingApplierScript.register_class_binding(control, expr, registry, state, scope, binding_tag, restyle_cb)
				else:
					GtmlBindingApplierScript.register_attr_binding(control, cls["target"], expr, registry, state, scope, binding_tag)
			"v-show":
				var v_show_expr: Dictionary = GtmlBindingExprScript.parse(node.attrs[attr_name])
				GtmlBindingApplierScript.register_v_show(control, v_show_expr, registry, state, scope, binding_tag)
			"v-model":
				...
				GtmlBindingApplierScript.register_v_model(target_ctl, v_model_key, registry, state, scope, binding_tag)
			"v-on":
				...
				var raw_ast: Dictionary = GtmlBindingExprScript.parse(raw_value)
				if raw_ast.get("type") == "call":
					GtmlBindingApplierScript.register_event_with_args(control, cls["target"], raw_ast, state, scope, _gtml_view, binding_tag)
			_:
				pass
...
func _maybe_expand_v_for_children(parent_node) -> void:
	if _gtml_view == null or _gtml_view.state == null:
		return
	...
	for child in originals:
		if not child.is_text_node and child.attrs.has("v-for"):
			var spec = GtmlBindingApplierScript.parse_v_for(child.attrs["v-for"])
			if spec == null:
				push_warning("GtmlRenderer: invalid v-for expression: %s" % child.attrs["v-for"])
				continue
			...
			if arr == null:
				...
				continue
			if not (arr is Array):
				push_warning("GtmlRenderer: v-for source '%s' is not an Array" % spec["array_key"])
				continue
			...
			if not key_expr_src.is_empty():
				key_expr = GtmlBindingExprScript.parse(key_expr_src)
			...
			for i in (arr as Array).size():
				...
				if seen_keys.has(k):
					GtmlBindingApplierScript._warn("v-for duplicate key on initial render: %s" % k)
					dup = true
					break
				seen_keys[k] = true
...
func _clone_dom_node(node):
	var GtmlNode = preload("res://addons/gtml/src/html_parser/GtmlNode.gd")
	var clone
	if node.is_text_node:
		clone = GtmlNode.create_text(node.text)
	else:
		clone = GtmlNode.create_element(node.tag, node.attrs.duplicate())
	if _styles.has(node):
...
func _dispatch(node, ctx: Dictionary) -> Dictionary:
	match node.tag:
		...
		"p":
			return GtmlTextBuilder.build_paragraph(node, ctx)
		"span":
			return GtmlTextBuilder.build_span(node, ctx)
		"h1", "h2", "h3", "h4", "h5", "h6":
			var level: int = int(node.tag.substr(1))
			return GtmlTextBuilder.build_heading(node, level, ctx)
		"label":
			return GtmlTextBuilder.build_label(node, ctx)
		"strong", "b":
			var ctl = GtmlTextBuilder.build_bold(node, ctx)
			return {"control": ctl, "inner": ctl}
		"em", "i":
			var ctl2 = GtmlTextBuilder.build_italic(node, ctx)
			return {"control": ctl2, "inner": ctl2}
		"button":
			return GtmlButtonBuilder.build_button(node, ctx)
		"input":
			return GtmlInputBuilder.build_input(node, ctx)
		"textarea":
			return GtmlInputBuilder.build_textarea(node, ctx)
		"select":
			return GtmlInputBuilder.build_select(node, ctx)
		"option":
			return {"control": null, "inner": null}  # handled by parent <select>
		"img":
			return GtmlMediaBuilder.build_image(node, ctx)
		"br":
			var br_ctl = GtmlMediaBuilder.build_line_break()
			return {"control": br_ctl, "inner": br_ctl}
		"hr":
			return GtmlMediaBuilder.build_horizontal_rule(node, ctx)
		"progress":
			return GtmlMediaBuilder.build_progress(node, ctx)
		"svg":
			return GtmlMediaBuilder.build_svg(node, ctx)
		"ul":
			return GtmlListBuilder.build_list(node, false, ctx)
		"ol":
			return GtmlListBuilder.build_list(node, true, ctx)
		"li":
			return GtmlListBuilder.build_list_item(node, ctx)
		"a":
			return GtmlAnchorBuilder.build_anchor(node, ctx)
		_:
...
func _build_text_node(node) -> Control:
	...
	if "{{" in text and _gtml_view != null and _gtml_view.state != null:
		var registry = _gtml_view._binding_registry
		if registry != null:
			var scope: Dictionary = node.get_meta("_binding_scope", {})
			var tag: String = node.get_meta("_vfor_tag", "")
			var spans: Array = GtmlBindingParserScript.find_interpolations(text)
			GtmlBindingApplierScript.register_text_interpolation(label, spans, registry, _gtml_view.state, scope, tag)

	return label


## Register an element with the GtmlView so consumers can look it up.
##   - ``aria-node`` becomes Control.name (accessible identification)
##   - ``title`` becomes tooltip_text
##   - ``id`` is used for GtmlView.get_element_by_id() lookups
func _register_element_with_id(inner_control: Control, wrapper_control: Control, node) -> void:
	...
	if _gtml_view != null:
		_gtml_view.register_element(id, inner_control, wrapper_control)
...
## Layout/dimension props are GtmlDimensions' job; container chrome is GtmlWrap's.
...
## Thin shim around GtmlWrap so element builders keep their existing call site.
func _wrap_with_margin_padding(control: Control, style: Dictionary) -> Control:
	return GtmlWrap.apply(control, style)
...
func _compute_vfor_key(key_expr: Variant, item: Variant, index: int, spec: Dictionary, state, parent_scope: Dictionary) -> String:
	...
	if key_expr is Dictionary and key_expr.get("type") == "error":
		GtmlBindingApplierScript._warn("v-for :key parse error: %s" % key_expr.get("message", ""))
		return str(index)
	var item_scope: Dictionary = parent_scope.duplicate()
	item_scope[spec["loop_var"]] = item
	if spec["index_var"] != "":
		item_scope[spec["index_var"]] = index
	var v = GtmlBindingApplierScript.eval(key_expr, state, item_scope)
	if v == null:
		...
		GtmlBindingApplierScript._warn("v-for :key resolved to null at index %d; falling back to index" % index)
		return str(index)
	return str(v)
...
func _reconcile_v_for_region(parent_node, container: Control) -> void:
	...
	for template_id in state_map.keys():
		...
		for i in (arr as Array).size():
			...
			var k: String = _compute_vfor_key(key_expr, item, i, spec, state, parent_scope)
			if seen_keys.has(k):
				GtmlBindingApplierScript._warn("v-for duplicate key during reconcile: %s" % k)
				has_dup = true
				break
			seen_keys[k] = true
			new_keys.append(k)
		...
		if _packed_strings_equal(old_keys, new_keys):
			...
			var ops: Array = GtmlVForReconciler.diff(old_keys, new_keys)
	...
	if _gtml_view != null and _gtml_view._content_root != null:
		var GtmlFocusManagerScript = preload("res://addons/gtml/src/focus/GtmlFocusManager.gd")
		GtmlFocusManagerScript.wire_focus(_gtml_view._content_root)
...
func _fire_clone_bindings(registry, tag: String) -> void:
	...
	for b in registry._by_tag[tag]:
		if GtmlBindingRegistry._is_alive(b):
			b["apply"].call()
...
func _make_class_restyle_callback(control: Control, node) -> Callable:
	if _gtml_view == null:
		return Callable()
	var css_rules: Array = _gtml_view._css_rules
	if css_rules.is_empty():
		return Callable()
	var chain: Array = _current_chain.duplicate()
	var tm = _transition_manager
	var GtmlClassRestylerScript = preload("res://addons/gtml/src/css/GtmlClassRestyler.gd")
	# Base snapshot = static-only visual props.
	var static_classes: PackedStringArray = node.get_classes()
	var base_snapshot: Dictionary = GtmlClassRestylerScript.resolve_visual_props(node, chain, static_classes, css_rules)
	var ctrl_ref: WeakRef = weakref(control)
	return func(dynamic_classes: PackedStringArray):
		var c = ctrl_ref.get_ref()
		if c == null:
			return
		GtmlClassRestylerScript.restyle(c, node, chain, dynamic_classes, css_rules, base_snapshot, tm)


## Classify focusability for an element and stamp _gtml_* focus meta +
...
func _stamp_focus_meta(node, control: Control, inner: Control) -> void:
	...
	if node.has_attr("focus-trap"):
		control.set_meta("_gtml_focus_trap", true)
	...
	focus_target.set_meta("_gtml_focusable", true)
	focus_target.set_meta("_gtml_tabindex", tabindex)
	focus_target.set_meta("_gtml_tab_skip", has_tabindex and tabindex == -1)
	focus_target.focus_mode = Control.FOCUS_ALL
	if node.has_attr("autofocus"):
		focus_target.set_meta("_gtml_autofocus", true)
