...
##
## Percentage dimensions are recomputed whenever the containing GmlView (or,
...
##
## Meta keys consumed (set by GmlDimensions during initial style application):
...
static func _setup(control: Control) -> void:
	...
	var view := find_gml_view(control)
...
## Find the nearest GmlView ancestor (or null) of a control.
static func find_gml_view(control: Control) -> Control:
	var current: Node = control.get_parent()
	while current != null:
		var s := current.get_script()
		if s != null and s.resource_path.ends_with("GmlView.gd"):
			return current
