...
static func _linear(bg_data: Dictionary, style: Dictionary) -> Control:
	...
	if colors.size() < 2:
		push_warning("GmlBackgrounds: linear-gradient needs at least 2 colors")
		return null
	...
	tex.width = 256
	tex.height = 256
	GmlStyles.apply_gradient_angle(tex, angle)
...
static func _radial(bg_data: Dictionary, style: Dictionary) -> Control:
	...
	var offsets: Array = bg_data.get("offsets", [])
	if colors.size() < 2:
		push_warning("GmlBackgrounds: radial-gradient needs at least 2 colors")
		return null
...
static func _image(bg_data: Dictionary) -> Control:
	...
	if not ResourceLoader.exists(url):
		push_warning("GmlBackgrounds: image not found: %s" % url)
		return null

	var texture := load(url) as Texture2D
	if texture == null:
		push_warning("GmlBackgrounds: failed to load image: %s" % url)
		return null
