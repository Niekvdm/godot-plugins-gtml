...
static func build_paragraph_inner(node, ctx: Dictionary) -> Control:
	...
	GtmlStyles.apply_text_color(label, style, defaults)
	GtmlStyles.apply_text_styles(label, style, defaults)

	var result: Control = label

	# Apply text-shadow if present (shadow goes behind, so apply first)
	if style.has("text-shadow"):
		result = GtmlStyles.apply_text_shadow(label, style["text-shadow"])
	...
	if style.has("text-decoration"):
		...
		if result == label:
			result = GtmlStyles.apply_text_decoration(label, style["text-decoration"], color)
...
static func build_span(node, ctx: Dictionary) -> Dictionary:
	...
	if style.has("text-shadow"):
		result_control = GtmlStyles.apply_text_shadow(label, style["text-shadow"])

	# Apply text-decoration if present
	if style.has("text-decoration"):
		var color: Color = style.get("color", defaults.get("default_font_color", Color.WHITE))
		if result_control == label:
			result_control = GtmlStyles.apply_text_decoration(label, style["text-decoration"], color)
...
##
## CheckBox and radio respect a CSS :focus rule (see GtmlInputBuilder); when
...
static func _build_label_as_container(node, ctx: Dictionary) -> Control:
	...
	for child in node.children:
		if child.is_text_node:
			...
			GtmlStyles.apply_text_color(text_label, style, defaults)
			GtmlStyles.apply_text_styles(text_label, style, defaults)
