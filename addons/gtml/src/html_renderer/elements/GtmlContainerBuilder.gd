class_name GtmlContainerBuilder
extends RefCounted
...
## ctx contains: styles, defaults, gtml_view, build_node (Callable), get_style (Callable), wrap_with_margin_padding (Callable)
...
static func build_div_inner(node, ctx: Dictionary) -> Control:
	...
	if style.has("justify-content") and container is BoxContainer:
		container.alignment = GtmlStyles.parse_box_alignment(style["justify-content"])
	...
	for child in node.children:
		...
		if child_control != null:
			...
			if child.is_text_node and child_control is Label:
				GtmlStyles.apply_text_color(child_control, style, defaults)
				GtmlStyles.apply_text_styles(child_control, style, defaults)
			...
			if not child_align.is_empty() and child_align != "auto":
				GtmlStyles.apply_cross_axis_alignment(child_control, child_align, is_row, child_style)
