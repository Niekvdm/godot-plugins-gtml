...
static func build_input(node, ctx: Dictionary) -> Dictionary:
	...
	match input_type:
		"text", "password", "email", "number":
			inner = _build_text_input(node, input_type, style, gtml_view, defaults)
		"checkbox":
			inner = _build_checkbox_input(node, style, gtml_view, defaults)
		"radio":
			inner = _build_radio_input(node, style, gtml_view, defaults)
		"range":
			inner = _build_range_input(node, style, gtml_view, defaults)
		"submit":
			inner = _build_submit_button(node, style, ctx)
		_:
			inner = _build_text_input(node, "text", style, gtml_view, defaults)
...
static func _build_text_input(node, input_type: String, style: Dictionary, gtml_view, defaults: Dictionary = {}) -> Control:
	...
	if gtml_view != null and not input_id.is_empty():
...
static func _build_checkbox_input(node, style: Dictionary, gtml_view, defaults: Dictionary = {}) -> Control:
...
static func _build_radio_input(node, style: Dictionary, gtml_view, defaults: Dictionary = {}) -> Control:
	...
	if gtml_view != null and not group_name.is_empty():
		var button_group = gtml_view.get_radio_group(group_name)
...
## Connect the input's gui_input signal to emit GtmlView.key_pressed when an
## ``@keydown="handler"`` attribute is present. Filters to actual key-down
## events (skips key-up, mouse, and other input event types).
static func _wire_keydown(control: Control, node, gtml_view) -> void:
	if gtml_view == null:
		return
...
static func _apply_button_checked_stylebox(btn: CheckBox, style: Dictionary) -> void:
	...
	if checked_style.has("border") or checked_style.has("border-width") or checked_style.has("border-color"):
		GtmlStyles.apply_border_to_stylebox(box, checked_style)
		has_visual = true
...
static func _apply_button_focus_stylebox(btn: CheckBox, style: Dictionary) -> void:
	...
	if focus_style.has("border") or focus_style.has("border-width") or focus_style.has("border-color"):
		GtmlStyles.apply_border_to_stylebox(box, focus_style)
		has_visual = true
...
static func _build_range_input(node, style: Dictionary, gtml_view, defaults: Dictionary = {}) -> Control:
...
static func _build_select_inner(node, ctx: Dictionary) -> Control:
	...
	if gtml_view != null and not select_id.is_empty():
		var view_ref = weakref(gtml_view)
