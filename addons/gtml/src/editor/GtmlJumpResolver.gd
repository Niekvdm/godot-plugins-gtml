...
static func _resolve_html(ctx: GtmlEditorContext) -> Variant:
...
static func _resolve_css(ctx: GtmlEditorContext) -> Variant:
	var line := ctx.line_at(ctx.cursor_line)
