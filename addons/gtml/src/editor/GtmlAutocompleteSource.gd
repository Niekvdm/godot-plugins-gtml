class_name GtmlAutocompleteSource
extends RefCounted
...
static func get_candidates(ctx: GtmlEditorContext) -> Array:
	if ctx.kind == "html":
...
static func _html_candidates(ctx: GtmlEditorContext) -> Array:
...
static func _css_candidates(ctx: GtmlEditorContext) -> Array:
	var prefix := ctx.prefix_at_cursor()
...
static func _absolute_offset(ctx: GtmlEditorContext) -> int:
	var off := 0
