class_name GtmlHtmlParser
extends RefCounted

## HTML parser for GML.
## Parses a subset of HTML into a GtmlNode tree.
##
...
## Mirrors GtmlCssParser.get_warnings() so editor tooling can surface both
...
func _warn(msg: String) -> void:
	...
	push_warning("GtmlHtmlParser [%d:%d]: %s" % [lc["line"], lc["col"], msg])
...
func parse(html: String):
	...
	var root = GtmlNodeScript.create_element("_root")
	...
	if root.children.size() > 1:
		var wrapper = GtmlNodeScript.create_element("div")
...
func _parse_element():
	...
	_depth += 1
	if _depth > MAX_DEPTH:
		push_error("GtmlHtmlParser: Maximum nesting depth exceeded (%d)" % MAX_DEPTH)
		_depth -= 1
		return null
	...
	var node = GtmlNodeScript.create_element(tag_name, attributes)
...
func _parse_text():
	...
	return GtmlNodeScript.create_text(text)
