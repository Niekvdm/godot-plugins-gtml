class_name GtmlBackgroundValues
extends RefCounted
...
static func parse_background(value: String) -> Dictionary:
	...
	# Fallback to color
	return {"type": "color", "color": GtmlColorValues.parse_color(value)}
...
static func parse_color_stop(stop: String) -> Dictionary:
	...
	if parts.size() == 1:
		return {"color": GtmlColorValues.parse_color(parts[0])}

	# Color with offset
	var color = GtmlColorValues.parse_color(parts[0])
