...
static func parse_color(value: String) -> Color:
	...
	if value.begins_with("rgba(") and value.ends_with(")"):
		...
		push_warning("GtmlColorValues: Invalid rgba() format '%s'" % value)
		return Color.WHITE
	...
	if value.begins_with("rgb(") and value.ends_with(")"):
		...
		push_warning("GtmlColorValues: Invalid rgb() format '%s'" % value)
		return Color.WHITE
...
static func parse_named_color(name: String) -> Color:
	match name.to_lower():
		...
		_:
			push_warning("GtmlColorValues: Unknown color '%s'" % name)
			return Color.WHITE
