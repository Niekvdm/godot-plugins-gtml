class_name GtmlTransformValues
extends RefCounted
...
static func parse_transform(raw: String) -> Dictionary:
	...
	while i < n:
		...
		if i >= n or s[i] != "(":
			push_warning("GtmlTransformValues: expected '(' after '%s' in '%s'" % [fn_name, raw])
			break
		var args_start := i + 1
		var close := s.find(")", args_start)
		if close < 0:
			push_warning("GtmlTransformValues: unterminated function '%s(' in '%s'" % [fn_name, raw])
			break
...
static func _apply_function(out: Dictionary, name: String, args_raw: String) -> void:
	...
	match name:
		...
		_:
			push_warning("GtmlTransformValues: unsupported transform function '%s'" % name)
