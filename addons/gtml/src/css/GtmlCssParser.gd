...
func _warn(msg: String) -> void:
	...
	push_warning("GtmlCssParser [%d:%d]: %s" % [lc["line"], lc["col"], msg])
...
##
## v0.2: a rule carries a structured ``GtmlSelector.Selector`` and the resolver
...
class CssRule:
	var selector = null              # GtmlSelector.Selector
...
## Delegates selector parsing to GtmlSelector so combinators + attribute
...
func _parse_rules_group() -> Array:
	...
	var selectors: Array = GtmlSelector.parse_group(selector_text)
...
static func _convert_property_value_static(prop_name: String, value: String):
	...
	if prop_name in SIZE_PROPS:
		return GtmlDimensionValues.parse_size(value)

	# Dimension properties (return Dictionary with value and unit)
	if prop_name in DIMENSION_PROPS:
		return GtmlDimensionValues.parse_dimension(value)

	# Color properties
	if prop_name in COLOR_PROPS:
		return GtmlColorValues.parse_color(value)

	# Border shorthand properties
	if prop_name in BORDER_PROPS:
		return GtmlBorderValues.parse_border(value)

	# Float properties
	if prop_name in FLOAT_PROPS:
		return GtmlDimensionValues.parse_float(value)

	# Font properties
	if prop_name == "font-family":
		return GtmlFontValues.parse_font_family(value)

	if prop_name == "font-weight":
		return GtmlFontValues.parse_font_weight(value)

	if prop_name == "letter-spacing":
		return GtmlFontValues.parse_letter_spacing(value)

	# Text decoration (can have multiple values like "underline line-through")
	if prop_name == "text-decoration":
		return GtmlFontValues.parse_text_decoration(value)

	# Background properties
	if prop_name == "background" or prop_name == "background-image":
		return GtmlBackgroundValues.parse_background(value)

	# Box shadow
	if prop_name == "box-shadow":
		return GtmlBorderValues.parse_box_shadow(value)

	# Text shadow
	if prop_name == "text-shadow":
		return GtmlBorderValues.parse_text_shadow(value)

	# Outline
	if prop_name == "outline":
		return GtmlBorderValues.parse_outline(value)

	# Transform: translate/scale/rotate composite
	if prop_name == "transform":
		return GtmlTransformValues.parse_transform(value)

	# Transition properties
	if prop_name == "transition":
		return GtmlTransitionValues.parse_transition(value)
	if prop_name == "transition-property":
		return GtmlTransitionValues.parse_transition_property(value)
	if prop_name == "transition-duration":
		return GtmlTransitionValues.parse_transition_duration(value)
	if prop_name == "transition-timing-function":
		return GtmlTransitionValues.parse_transition_timing_function(value)
	if prop_name == "transition-delay":
		return GtmlTransitionValues.parse_transition_delay(value)
