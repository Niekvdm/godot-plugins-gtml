...
## Pure-data tests for GtmlVForReconciler.diff(old_keys, new_keys).
...
func test_diff_identical_arrays_emits_no_ops() -> void:
	var ops: Array = GtmlVForReconciler.diff(_keys(["a", "b", "c"]), _keys(["a", "b", "c"]))
	assert_eq(ops.size(), 0)


func test_diff_both_empty_emits_no_ops() -> void:
	var ops: Array = GtmlVForReconciler.diff(_keys([]), _keys([]))
	assert_eq(ops.size(), 0)
...
func test_diff_all_append() -> void:
	var ops: Array = GtmlVForReconciler.diff(_keys(["a", "b"]), _keys(["a", "b", "c", "d"]))
...
func test_diff_all_prepend() -> void:
	var ops: Array = GtmlVForReconciler.diff(_keys(["c", "d"]), _keys(["a", "b", "c", "d"]))
...
func test_diff_all_remove() -> void:
	var ops: Array = GtmlVForReconciler.diff(_keys(["a", "b", "c"]), _keys([]))
...
func test_diff_middle_remove() -> void:
	var ops: Array = GtmlVForReconciler.diff(_keys(["a", "b", "c", "d"]), _keys(["a", "c", "d"]))
...
func test_diff_single_move() -> void:
	...
	var ops: Array = GtmlVForReconciler.diff(_keys(["a", "b", "c"]), _keys(["c", "a", "b"]))
...
func test_diff_reverse() -> void:
	...
	var ops: Array = GtmlVForReconciler.diff(_keys(["a", "b", "c", "d"]), _keys(["d", "c", "b", "a"]))
	var move_ops: Array = ops.filter(func(o): return o["op"] == "move")
...
func test_diff_replace_one() -> void:
	var ops: Array = GtmlVForReconciler.diff(_keys(["a", "b", "c"]), _keys(["a", "x", "c"]))
	assert_eq(ops.size(), 2)
...
func test_diff_common_prefix_middle_change() -> void:
	...
	var ops: Array = GtmlVForReconciler.diff(_keys(["a", "b", "c", "d"]), _keys(["a", "b", "x", "d"]))
...
func test_diff_common_suffix() -> void:
	...
	var ops: Array = GtmlVForReconciler.diff(_keys(["a", "b", "c", "d"]), _keys(["x", "b", "c", "d"]))
	assert_eq(ops.size(), 2)
...
func test_diff_empty_to_populated() -> void:
	var ops: Array = GtmlVForReconciler.diff(_keys([]), _keys(["a", "b"]))
	assert_eq(ops.size(), 2)
	for o in ops:
...
func test_diff_populated_to_empty() -> void:
	var ops: Array = GtmlVForReconciler.diff(_keys(["a", "b"]), _keys([]))
	assert_eq(ops.size(), 2)
	for o in ops:
		assert_eq(o["op"], "remove")
...
func test_diff_duplicate_keys_in_new_returns_empty_and_warns() -> void:
	...
	var ops: Array = GtmlVForReconciler.diff(_keys(["a"]), _keys(["a", "b", "b"]))
...
func test_diff_lis_keeps_increasing_subsequence_in_place() -> void:
	...
	var ops: Array = GtmlVForReconciler.diff(_keys(["1", "2", "3", "4", "5"]), _keys(["3", "1", "2", "4", "5"]))
	var move_ops: Array = ops.filter(func(o): return o["op"] == "move")
