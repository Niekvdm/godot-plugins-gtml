...
## Tests for GtmlAutocompleteSource — the engine that, given a buffer +
...
func _html_ctx(text: String, line: int, col: int) -> GtmlEditorContext:
	return GtmlEditorContext.from_html(text, line, col, "")


func _css_ctx(text: String, line: int, col: int, html: String = "") -> GtmlEditorContext:
	return GtmlEditorContext.from_css(text, line, col, html)
...
func test_empty_buffer_returns_no_candidates() -> void:
	assert_eq(GtmlAutocompleteSource.get_candidates(_html_ctx("", 0, 0)).size(), 0)
	assert_eq(GtmlAutocompleteSource.get_candidates(_css_ctx("", 0, 0)).size(), 0)
...
func test_html_tag_completion_after_open_bracket() -> void:
	...
	var candidates = GtmlAutocompleteSource.get_candidates(ctx)
...
func test_html_tag_completion_filters_by_typed_prefix() -> void:
	...
	var labels = _labels(GtmlAutocompleteSource.get_candidates(ctx))
...
func test_class_completion_lists_classes_declared_in_other_buffer() -> void:
	...
	var html := '<div class="'
	var css := ".card {}\n.row {}\n#x {}"
	var ctx = GtmlEditorContext.from_html(html, 0, html.length(), css)
	var labels = _labels(GtmlAutocompleteSource.get_candidates(ctx))
