...
## Tests for GtmlBindingApplier — evaluates AST + writes to controls.


func _state(values: Dictionary = {}) -> GtmlState:
	var s := GtmlState.new()
	for k in values:
		s.set(k, values[k])
	return s
...
func test_eval_path_returns_state_value() -> void:
	...
	assert_eq(GtmlBindingApplier.eval(ast, s, {}), 42)
...
func test_eval_dotted_path_reads_flat_key() -> void:
	...
	assert_eq(GtmlBindingApplier.eval(ast, s, {}), 85)
...
func test_register_text_interpolation_initial_apply_writes_label() -> void:
	...
	GtmlBindingApplier.register_text_interpolation(label, spans, r, s)
...
func test_register_attr_binding_disabled_writes_bool() -> void:
	...
	var ast: Dictionary = GtmlBindingExpr.parse("locked")
	GtmlBindingApplier.register_attr_binding(btn, "disabled", ast, r, s)
...
func test_register_attr_binding_value_writes_text() -> void:
	...
	var ast: Dictionary = GtmlBindingExpr.parse("name")
	GtmlBindingApplier.register_attr_binding(line, "value", ast, r, s)
...
func test_register_attr_binding_unknown_target_is_noop() -> void:
	...
	var ast: Dictionary = GtmlBindingExpr.parse("x")
	GtmlBindingApplier.register_attr_binding(ctrl, "unrecognized_attr", ast, r, s)
...
func test_eval_object_returns_filtered_keys_by_truthy_values() -> void:
	...
	var ast: Dictionary = GtmlBindingExpr.parse("{ active: is_active, dim: is_dim }")
	var result = GtmlBindingApplier.eval(ast, s, {})
...
func test_eval_object_with_negation() -> void:
	...
	var ast: Dictionary = GtmlBindingExpr.parse("{ ready: !is_loading }")
	var result = GtmlBindingApplier.eval(ast, s, {})
	assert_eq(result.size(), 1)
...
func test_eval_array_returns_concatenated_strings() -> void:
	...
	var ast: Dictionary = GtmlBindingExpr.parse("['static', dyn]")
	var result = GtmlBindingApplier.eval(ast, s, {})
	assert_true(result is PackedStringArray)
...
func test_register_class_binding_writes_meta_classes() -> void:
	...
	var ctrl := Control.new()
	add_child_autofree(ctrl)
	var ast: Dictionary = GtmlBindingExpr.parse("{ active: is_on }")
	GtmlBindingApplier.register_class_binding(ctrl, ast, r, s)
...
func test_on_warning_callable_receives_message_when_set() -> void:
	var captured: Array = []
	GtmlBindingApplier._on_warning = func(msg): captured.append(msg)
	GtmlBindingApplier._warn("test message")
	assert_eq(captured.size(), 1)
	assert_eq(captured[0], "test message")
	GtmlBindingApplier._on_warning = Callable()   # reset
...
func test_warn_falls_back_to_push_warning_when_unset() -> void:
	...
	GtmlBindingApplier._warn("hello")
	assert_true(true)
...
func test_eval_array_index_returns_element() -> void:
	...
	var ast: Dictionary = GtmlBindingExpr.parse("items[1]")
	assert_eq(GtmlBindingApplier.eval(ast, s, {}), "b")


func test_eval_dict_index_returns_value() -> void:
	var s := _state({"map": {"k": "v"}})
	var ast: Dictionary = GtmlBindingExpr.parse("map['k']")
	assert_eq(GtmlBindingApplier.eval(ast, s, {}), "v")
...
func test_eval_index_out_of_bounds_returns_null_no_warn() -> void:
	...
	var ast: Dictionary = GtmlBindingExpr.parse("items[5]")
	assert_null(GtmlBindingApplier.eval(ast, s, {}))
...
func test_eval_unary_minus_number() -> void:
	...
	assert_eq(GtmlBindingApplier.eval(ast, _state(), {}), -5.0)
...
func test_eval_unary_minus_string_warns_returns_null() -> void:
	...
	GtmlBindingApplier._on_warning = func(m): captured.append(m)
	var ast: Dictionary = GtmlBindingExpr.parse("-name")
	assert_null(GtmlBindingApplier.eval(ast, _state({"name": "Ada"}), {}))
...
func test_eval_add_numbers() -> void:
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("2 + 3"), _state(), {}), 5.0)


func test_eval_subtract() -> void:
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("10 - 4"), _state(), {}), 6.0)


func test_eval_multiply() -> void:
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("4 * 5"), _state(), {}), 20.0)


func test_eval_divide() -> void:
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("20 / 4"), _state(), {}), 5.0)


func test_eval_modulo() -> void:
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("10 % 3"), _state(), {}), 1)
...
func test_eval_divide_int_int_returns_float_no_truncation() -> void:
	...
	var ast: Dictionary = GtmlBindingExpr.parse("hp / max")
	assert_eq(GtmlBindingApplier.eval(ast, s, {}), 2.5)


func test_eval_divide_by_zero_warns_returns_null() -> void:
	var captured: Array = []
	GtmlBindingApplier._on_warning = func(m): captured.append(m)
	assert_null(GtmlBindingApplier.eval(GtmlBindingExpr.parse("5 / 0"), _state(), {}))
	assert_gt(captured.size(), 0)
	GtmlBindingApplier._on_warning = Callable()
...
func test_eval_string_concat_num_string_warns() -> void:
	...
	assert_null(GtmlBindingApplier.eval(GtmlBindingExpr.parse("5 + name"), _state({"name": "x"}), {}))
	assert_gt(captured.size(), 0)
	GtmlBindingApplier._on_warning = Callable()
...
func test_eval_greater_than_numbers() -> void:
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("5 > 3"), _state(), {}), true)


func test_eval_less_than_strings_lex() -> void:
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("'a' < 'b'"), _state(), {}), true)


func test_eval_equality_same_value() -> void:
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("1 == 1"), _state(), {}), true)
...
func test_eval_equality_cross_type_no_warn() -> void:
	...
	GtmlBindingApplier.eval(GtmlBindingExpr.parse("selected_item == null"), s, {})
...
func test_eval_comparison_cross_type_warns() -> void:
	var captured: Array = []
	GtmlBindingApplier._on_warning = func(m): captured.append(m)
	var ast: Dictionary = GtmlBindingExpr.parse("5 > 'a'")
	assert_null(GtmlBindingApplier.eval(ast, _state(), {}))
	assert_gt(captured.size(), 0)
	GtmlBindingApplier._on_warning = Callable()
...
func test_eval_and_short_circuits_on_falsy_left() -> void:
	...
	var s := _state({"a": 0})
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("a && missing"), s, {}), 0)


func test_eval_or_returns_first_truthy() -> void:
	var s := _state({"a": 0, "b": "found"})
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("a || b"), s, {}), "found")


func test_eval_and_returns_right_when_left_truthy() -> void:
	var s := _state({"a": 1, "b": "x"})
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("a && b"), s, {}), "x")
...
func test_eval_ternary_true_branch() -> void:
	var s := _state({"x": true})
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("x ? 'yes' : 'no'"), s, {}), "yes")


func test_eval_ternary_false_branch() -> void:
	var s := _state({"x": false})
	assert_eq(GtmlBindingApplier.eval(GtmlBindingExpr.parse("x ? 'yes' : 'no'"), s, {}), "no")


# ─── Dep collection for new AST nodes (Task 9) ─────────────

func test_collect_deps_binop() -> void:
	var ast: Dictionary = GtmlBindingExpr.parse("hp + max_hp")
	var deps: Array = []
	GtmlBindingApplier._collect_expr_deps(ast, deps)
